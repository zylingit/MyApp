...
public class ColorPickerProgressBar extends View {
    ...
    public ColorPickerProgressBar(Context context) {
        super(context);
    }

    public ColorPickerProgressBar(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ColorPickerProgressBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    // 新增的自定义宽度和高度
    private int customWidth = -1; // 默认值为 -1 表示未设置
    private int customHeight = -1;
    /**
     * 指示点半径，默认为颜色条宽度的 4/3。
     */
    private int customIndicatorRadius = -1; // -1 表示未设置
    ...
    public void setPickerSize(int width, int height) {
        if (width > 0 && height > 0) {
            ...
            requestLayout(); // 重新布局视图
        }
    }

    /**
     * 设置自定义的指示点半径。
     * @param radius 指示点的半径，单位为像素。
     */
    public void setCustomIndicatorRadius(int radius) {
        if (radius > 0) {
            this.customIndicatorRadius = radius;
    ...
    private void calculBounds() {
        ...
        // 如果用户没有设置自定义的指示点半径，使用默认值
        if (customIndicatorRadius > 0) {
            mRadius = customIndicatorRadius;
        } else {
            mRadius = each * 7 / 2;
        }
     ...
     * 如果想设置 alpha ，可以在{@link OnColorPickerChangeListener#onColorChanged(ColorPickerProgressBar, int)} 回调
    ...
    private void createIndicatorBitmap() {
        int radius = (customIndicatorRadius > 0) ? customIndicatorRadius : mRadius;

        paintForIndicator.setColor(Color.argb(128, 0, 0, 0));
        paintForIndicator.setStyle(Paint.Style.STROKE);
        paintForIndicator.setStrokeWidth(2F);
        paintForIndicator.clearShadowLayer();

        Canvas c = new Canvas(bitmapForIndicator);
        c.drawCircle(radius, radius, radius - 1, paintForIndicator);

        paintForIndicator.setStyle(Paint.Style.FILL);
        paintForIndicator.setColor(Color.TRANSPARENT);
        c.drawCircle(radius, radius, radius - 2, paintForIndicator);
    ...
    private int calcuColor() {
        ...
        Log.d("AAA","--------------------X:"+x+"--------bitmap-x:"+bitmapForColor.getWidth());
        if (x == bitmapForColor.getWidth()){
            x = x-1;
        }
    ...
    public interface OnColorPickerChangeListener {
         ...
         * @param color  颜色
         */
        void onColorChanged(ColorPickerProgressBar picker, int color);
         ...
         * @param picker ColorPickerView
         */
        void onStartTrackingTouch(ColorPickerProgressBar picker);
         ...
         * @param picker ColorPickerView
         */
        void onStopTrackingTouch(ColorPickerProgressBar picker);
    }
        ...
        if (orientation == Orientation.HORIZONTAL) {
            switch (keyCode) {
                case KeyEvent.KEYCODE_DPAD_LEFT:
                    moveIndicator(-1); // 向左移动指示点
                    return true;
                case KeyEvent.KEYCODE_DPAD_RIGHT:
                    moveIndicator(1); // 向右移动指示点
                    return true;
                default:
                    break;
            }
    ...
    private void moveIndicator(int delta) {
        if (orientation == Orientation.HORIZONTAL) {
            curX += delta;

            // 确保指示点不超出颜色条边界
            curX = Math.max(mLeft + mRadius, Math.min(curX, mRight - mRadius));
            curY = getHeight() / 2;
